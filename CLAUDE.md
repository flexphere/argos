...
分析は skill 実行時に親セッションが `references/SEMANTIC_PROMPT.md` の指示に従って in-context で 2 タスク (drift / misplacement) を生成する。出力は zod (`src/schema/semantic.ts`) で検証されてから fixture に焼き込まれる。ブラウザは結果を表示するだけ。
